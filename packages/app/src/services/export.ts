...
function generateSVGChart(
  ...
  return `
    <div class="chart-container">
      <h3>${escapeHTML(title)} (${escapeHTML(unit)})</h3>
      <svg width="${width}" height="${height}" style="border: 1px solid #ddd; background: #fff;">
        ...
        ${timeLabels.map((t: any) => `<text x="${t.x}" y="${height - 10}" font-size="9" fill="#666" text-anchor="middle">${escapeHTML(t.label)}</text>`).join('')}
      </svg>
    </div>
  `;
}

function buildPDFHTML(session: AnesthesiaSession, includeScript: boolean): string {
  const formatTime = (timestamp: string) => {
    const date = new Date(timestamp);
    return date.toLocaleString('zh-TW');
  };

  const h = escapeHTML; // 短別名
  const speciesLabel = SPECIES_LABELS[session.patientInfo.species] ?? session.patientInfo.species;
  ...
  return `
    ...
    <head>
      <meta charset="UTF-8">
      <title>麻醉記錄 - ${h(session.patientInfo.patientName)}</title>
      <style>
    ...
    <body>
      <h1>${h(session.patientInfo.hospitalName)}麻醉監測記錄表</h1>

      <h2>病患資料</h2>
      <table class="info-table">
        <tr><td>動物醫院</td><td>${h(session.patientInfo.hospitalName)}</td></tr>
        <tr><td>病患名稱</td><td>${h(session.patientInfo.patientName)}</td></tr>
        <tr><td>病例編號</td><td>${h(session.patientInfo.caseNumber)}</td></tr>
        <tr><td>動物種別</td><td>${h(speciesLabel)}</td></tr>
        ...
        <tbody>
          ${session.records.map(r => `
            <tr>
              ...
              <td class="notes">${h(r.notes || '')}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      ${includeScript ? '<script>window.onload = function() { window.print(); }</script>' : ''}
    </body>
    </html>
  `;
}
...
function exportPDFWeb(session: AnesthesiaSession): void {
  const html = buildPDFHTML(session, true);
...
function generatePDFHTML(session: AnesthesiaSession): string {
  return buildPDFHTML(session, false);
}
...
async function exportPDFNative(session: AnesthesiaSession): Promise<void> {
  ...
  // Sanitize caseNumber for filename safety
  const safeCaseNumber = sanitizeFileName(session.patientInfo.caseNumber);
  const fileName = `anesthesia_${safeCaseNumber}_${Date.now()}.pdf`;
...
async function exportCSVNative(session: AnesthesiaSession): Promise<void> {
  ...
  const safeCaseNumber = sanitizeFileName(session.patientInfo.caseNumber);
  const fileName = `anesthesia_${safeCaseNumber}_${Date.now()}.csv`;
